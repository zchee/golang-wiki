...
1. Manually gz compress the .wasm file - This reduces things reasonably well.  For example, the ~2MB (minimum file size) example Wasm will compress down to around 500kB.
  * Using https://github.com/google/zopfli[Zopfli] to do the gzip compression is probably the best gzip approach (currently)
